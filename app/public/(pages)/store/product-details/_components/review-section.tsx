        ...
        <div className="lg:col-span-4 lg:sticky lg:top-24 h-fit">
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star
                        ...
                        size={18}
                        className={
                          s <= Math.round(parseFloat(summary.averageRating))
                            ? "text-primary"
                            : "text-slate-200"
                        }
                        fill={
                ...
                {[5, 4, 3, 2, 1].map((s) => {
                  const count =
                    summary.ratingDistribution[
                      s as keyof ReviewSummary["ratingDistribution"]
                    ] || 0;
                  const percentage =
                  ...
                  return (
                    <div key={s} className="flex items-center gap-4">
                      <span className="text-sm font-semibold text-slate-700 w-3">
                        {s}
                      </span>
          ...
          <div className="mt-10 border-t border-slate-100 pt-10">
            ...
            <p className="mt-2 text-sm text-light-slate leading-relaxed">
              Have you used this product? Your clinical feedback helps other
        ...
        <div className="lg:col-span-8">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-black">
              User Feedback
              {meta ? (
                <span className="ml-2 font-normal text-light-slate">
                  ({meta.total})
                </span>
              ) : null}
            </h3>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 text-xs font-semibold transition ${
                        helpfulIds.has(review.id)
                          ? "text-primary"
                          : "text-light-slate hover:text-black"
                      }`}
                    >
                      <ThumbsUp size={14} />
                  ...
                  <button
                    ...
                    className={`h-10 w-10 rounded-xl font-bold transition ${
                      page === i + 1
                        ...
                        : "bg-slate-50 text-light-slate hover:bg-slate-100"
                    }`}
                  >
                    {i + 1}
                  </button>
