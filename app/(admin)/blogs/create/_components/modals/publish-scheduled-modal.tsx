  ...
  return (
    <ModalShell onClose={onClose}>
      <div className="relative w-full max-w-[520px] rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <h3 className="mt-4 text-center text-base font-extrabold text-slate-900">Article Scheduled Successfully</h3>

        <p className="mt-2 text-center text-xs leading-5 text-slate-500">
          Your clinical update has been successfully queued for publication. It will go live and notify subscribers at
          the selected time.
        </p>

        <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Article Title</p>
          <p className="mt-1 text-sm font-semibold text-slate-900">{title}</p>

          <div className="mt-3 grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Publish Date</p>
              <p className="mt-1 text-xs font-semibold text-slate-700">{publishDate}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Publish Time</p>
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          <GhostButton onClick={onViewSchedule} className="px-4">
            View Schedule
          </GhostButton>
          <PrimaryButton onClick={onReturnDashboard} className="px-4">
            Return to Dashboard
          </PrimaryButton>
        </div>
      </div>
