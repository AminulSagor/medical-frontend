...
function statusBadge(r: ProductRow) {
    ...
    return <span className={r.status === "active" ? active : draft}>{r.status === "active" ? "Active" : "Draft"}</span>;
}
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-lg font-extrabold text-slate-900">Top Selling Products</h1>
                <p className="text-xs text-slate-500">Full list ranked by total sales</p>
            </div>
                            ...
                            {pageRows.map((r) => {
                                ...
                                return (
                                    <tr key={r.id} className="text-sm text-slate-800 hover:bg-slate-50/50">
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="truncate text-[11px] text-slate-400">{r.updatedLabel}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-5 text-right font-semibold text-slate-900">
                                            {revenue == null ? <span className="text-slate-400">—</span> : money(revenue)}
                                        </td>
                ...
                <div className="flex flex-col gap-3 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="inline-flex items-center overflow-hidden rounded-md border border-slate-200 bg-white text-sm">
                        <button
                            className={[
                                "px-3 py-2",
                                canPrev ? "text-slate-600 hover:bg-slate-50" : "cursor-not-allowed text-slate-300",
                            ].join(" ")}
                        ...
                        <button
                            className={[
                                "px-3 py-2",
                                canNext ? "text-slate-600 hover:bg-slate-50" : "cursor-not-allowed text-slate-300",
                            ].join(" ")}
