...
export default function TopSellingProductsViewAllClient() {
  ...
  const stats = useMemo(() => {
    const totalUnits = filteredSorted.reduce(
      (sum, r) => sum + (r.sales ?? 0),
      0,
    );
    ...
    const avgOrderValue = totalUnits > 0 ? revenueSum / totalUnits : 0;
        ...
        <div className="relative" ref={filterWrapRef}>
          ...
          >
            <Filter
              size={16}
              className={
                category !== "all" ? "text-[var(--primary)]" : "text-slate-600"
              }
            />
