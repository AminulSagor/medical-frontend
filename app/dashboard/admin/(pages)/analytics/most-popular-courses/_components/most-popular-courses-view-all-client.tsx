...
import { ArrowLeft, Users, GraduationCap, BarChart3, Star } from "lucide-react";
import type {
...
function statusPill(label: string) {
  ...
  return (
    <span className={["rounded-full px-2 py-1 text-[11px] font-semibold ring-1", style].join(" ")}>
      {label}
    </span>
  );
}
...
export default function MostPopularCoursesViewAllClient({
  ...
  metrics: PopularCoursesMetricsResponse;
  ...
  const avgCompletion =
    items.length > 0
      ? Math.round(
          items.reduce((sum, item) => sum + completionToPercent(item.completion), 0) /
            items.length
        )
      : 0;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">Most Popular Courses</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-2xl font-extrabold text-slate-900">{metrics.totalEnrollments.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Active Workshops</p>
              <p className="mt-1 text-2xl font-extrabold text-slate-900">{metrics.activeWorkshops.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-2xl font-extrabold text-slate-900">{avgCompletion}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Courses in Page</p>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1100px] w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((r, index) => (
                <tr key={`${r.courseName}-${index}`} className="text-sm text-slate-800 hover:bg-slate-50/50">
                  <td className="px-6 py-5 font-semibold text-slate-900">{r.courseName}</td>
                  <td className="px-6 py-5 text-slate-600">{r.category}</td>
                  <td className="px-6 py-5 text-slate-600">{r.nextSession}</td>
                  <td className="px-6 py-5 text-slate-700">{r.instructorDetails.name}</td>
                  <td className="px-6 py-5 text-right font-semibold text-slate-900">{r.enrolled.toLocaleString()}</td>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-semibold text-slate-600">{r.completion}</span>
                    </div>
                  </td>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-10 text-sm text-slate-500">No courses found.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
