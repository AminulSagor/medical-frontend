...
import { Eye, FileText, MoreHorizontal, Search, Filter, ChevronDown } from "lucide-react";
...
const OPTIONS: Array<"all" | FulfillmentStatus> = [
    "all",
    "shipped",
    "processing",
    "received",
...
// deterministic “soft gradient” based on name
...
function Pagination({
    ...
    const pages = Array.from({ length: totalPages }, (_, i) => i + 1).slice(0, 7); // simple, clean
...
export default function OrdersTable({
    ...
    onStatusChange: (v: "all" | FulfillmentStatus) => void;
    page: number;
    pageSize: number;
    totalCount: number;
    ...
    const [hovered, setHovered] = useState<("all" | FulfillmentStatus) | null>(null);
    ...
    return (
        <div className="overflow-hidden">
            {/* Header row: title left, search+filter right (like your 2nd pic) */}
                ...
                <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center md:w-auto">
                    ...
                    <div className="relative" ref={wrapRef}>
                        ...
                        >
                            ...
                            {status === "all" ? "Filter Status" : status.charAt(0).toUpperCase() + status.slice(1)}
                        ...
                        {rows.map((r) => {
                            ...
                            const fulfill =
                                r.fulfillment === "shipped" ? (
                                    <DotStatus tone="green" label="SHIPPED" />
                                ) : r.fulfillment === "processing" ? (
                                    <DotStatus tone="orange" label="PENDING" />
                                ) : (
                                    <DotStatus tone="slate" label="RECEIVED" />
                                );
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* Customer avatar + name (like your 2nd pic) */}
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="font-semibold text-slate-900">
                                                {r.customer}
                                                <span className="sr-only">{initials(r.customer)}</span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-5 font-bold text-slate-900">{r.total}</td>

                                    <td className="px-6 py-5">
                                        <div className="flex items-center justify-center gap-2">
                                            <Link
                                                href={`/orders-and-sales/${encodeURIComponent(r.id.replace(/^#/, ""))}`}
            ...
            {/* Footer: showing range + working pagination */}
            <div className="flex flex-col gap-3 border-t border-slate-200 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs text-slate-500 tracking-wide">
                    SHOWING{" "}
                    <span className="font-bold text-slate-700">{from}</span> TO{" "}
