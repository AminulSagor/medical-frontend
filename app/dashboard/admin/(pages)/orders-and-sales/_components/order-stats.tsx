    ...
    >
      ...
      {showDots ? (
        <div className="absolute right-5 top-5 flex items-center gap-1">
          <span
            className={["h-2 w-2 rounded-full", accentDots[accent]].join(" ")}
          />
...
export default function OrderStats({
  ...
  const safeCards: AdminOrdersSummaryCards = {
    thisMonthRevenue: cards?.thisMonthRevenue ?? 0,
    totalOrders: cards?.totalOrders ?? 0,
    toBeShipped: cards?.toBeShipped ?? 0,
    avgOrderValue: cards?.avgOrderValue ?? 0,
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
      <StatCard
        ...
        value={isLoading ? "..." : formatCurrency(safeCards.thisMonthRevenue)}
        chip="+12%"
        accent="cyan"
      ...
      <StatCard
        title="Total orders"
        value={isLoading ? "..." : String(safeCards.totalOrders)}
        accent="indigo"
        icon={<ShoppingBag size={18} className="text-indigo-600" />}
      />
      <StatCard
        title="To be shipped"
        value={isLoading ? "..." : String(safeCards.toBeShipped)}
        accent="orange"
        showDots
      ...
      <StatCard
        ...
        value={isLoading ? "..." : formatCurrency(safeCards.avgOrderValue)}
        accent="purple"
        icon={
          <SquareChartGantt
            size={20}
